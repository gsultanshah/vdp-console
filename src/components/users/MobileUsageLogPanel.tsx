...
function formatMetadata(metadata: Record<string, unknown>) {
  ...
  if (typeof metadata.query === 'string' && metadata.query) {
    parts.push(`query: ${metadata.query}`);
  }
  if (metadata.resultCount != null) {
    parts.push(`results: ${String(metadata.resultCount)}`);
  }
  if (typeof metadata.workMode === 'string' && metadata.workMode) {
    parts.push(`mode: ${metadata.workMode}`);
  }
  if (typeof metadata.blockCode === 'string' && metadata.blockCode) {
    parts.push(`block: ${metadata.blockCode}`);
  }
  if (typeof metadata.voterName === 'string' && metadata.voterName) {
    parts.push(`voter: ${metadata.voterName}`);
  }
  if (typeof metadata.voterCnic === 'string' && metadata.voterCnic) {
    parts.push(`cnic: ${metadata.voterCnic}`);
  }
  if (metadata.voterCount != null) {
    parts.push(`voters: ${String(metadata.voterCount)}`);
  }
  if (typeof metadata.blocksSearched === 'string' && metadata.blocksSearched) {
    parts.push(`blocks: ${metadata.blocksSearched}`);
  }
  if (parts.length === 0) {
        ...
        <CardHeader className="flex flex-row items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Searches, sharing, parchi views, and block downloads with time and GPS location.
            </p>
          </div>
