...
function toDesign(doc: Record<string, unknown>): VoterParchiDesign {
  return {
    ...
    layoutMode: (doc.layoutMode as VoterParchiDesign['layoutMode']) ?? 'slots',
    parchiPerPage: Number(doc.parchiPerPage) || 3,
    slots: Array.isArray(doc.slots) ? (doc.slots as VoterParchiDesign['slots']) : [],
    canvas: doc.canvas ? (doc.canvas as VoterParchiDesign['canvas']) : null,
...
export async function createDesign(
  ...
  try {
    ...
    const doc = {
      ...
      layoutMode: input.layoutMode ?? 'slots',
